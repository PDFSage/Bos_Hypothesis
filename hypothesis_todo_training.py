...
class EnhancedMultiModalTransformerLM(nn.Module):
    def __init__(
        ...
        if use_retrieval and retrieval_docs is not None:
            self.retriever = Retriever(retrieval_docs)
    ...
    def _is_confident(self, logits: torch.Tensor, threshold: float = 0.5) -> bool:
    ...
    def generate(
        ...
        if self.use_retrieval:
            docs = self.retriever.retrieve(prompt, top_k=retrieval_top_k)
            prompt = " ".join(docs) + " " + prompt
        ids = tokenizer.encode(prompt, return_tensors="pt").to(next(self.parameters()).device)
        generated = ids
        for _ in range(max_new_tokens):
            logits, _ = self.forward(generated, images)
            if not self._is_confident(logits[:, -1, :], threshold=warning_threshold):
                print("[Warning] Model confidence low; answer may be incomplete or weak.")
